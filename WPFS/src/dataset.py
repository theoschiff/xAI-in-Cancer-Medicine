...
def load_your_custom_dataset():
    mat_data = spio.loadmat('./data/your_custom_dataset.mat')
    X = pd.DataFrame(mat_data['X'])
    Y = pd.DataFrame(mat_data['Y'].T)
    print(f"Loaded your custom dataset with {X.shape[0]} samples and {X.shape[1]} features")
    print(f"Labels: {Y.value_counts()}, {Y.shape}")
    X = X.to_numpy()
...
class CustomTestDataset(Dataset):
	def __init__(self, X) -> None:
		...
		print(f"X shape pred: {self.X.shape}")
...
class CustomPytorchDataset(Dataset):
	def __init__(self, X, y, transform=None) -> None:
		...
		print(f"X shape train: {self.X.shape}")
...
def create_data_module_test(args, embedding_type, embedding_size):
	mat_data = spio.loadmat('./data/predict.mat')
	X = pd.DataFrame(mat_data['X'])
	print(f"Loaded your custom dataset with {X.shape[0]} samples and {X.shape[1]} features")
	...
	return X
	# Preprocess the data for the embeddings
	if args.embedding_preprocessing == 'raw':
		X_for_embeddings = X
	elif args.embedding_preprocessing == 'z_score':
		X_for_embeddings = StandardScaler().fit_transform(X)
	elif args.embedding_preprocessing == 'minmax':
		X_for_embeddings = MinMaxScaler().fit_transform(X)
	else:
		raise Exception("embedding_preprocessing not supported")
	...
	if embedding_type == 'histogram':
		...
		Embedding similar to FsNet
		"""
		embedding_matrix = compute_histogram_embedding(args, X_for_embeddings, embedding_size)
		return torch.tensor(embedding_matrix.copy(), dtype=torch.float32, requires_grad=False)
	elif embedding_type=='feature_values':
		"""
		A gene's embedding are its patients gene expressions.
