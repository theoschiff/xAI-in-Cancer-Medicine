...
class GeneralNeuralNetwork(pl.LightningModule):
	...
	def compute_loss(self, y_true, y_hat, x, x_hat, sparsity_weights):
		losses = {}
		losses['cross_entropy'] = F.cross_entropy(input=y_hat, target=torch.tensor(y_true, dtype=torch.int64 ), weight=torch.tensor(self.args.class_weights, device=self.device))
	...
	def on_train_epoch_end(self):
		self.log_epoch_metrics(self.train_outputs, 'train')
