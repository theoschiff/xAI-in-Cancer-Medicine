...
def train(args):
	...
	#### Pytorch lightning training
		...
		data = create_data_module_test(args, args.wpn_embedding_type, args.wpn_embedding_size)
		print(f"Data for predictions: {data.shape}")	
  
		predictions = trainer.predict(model, dataloaders=DataLoader(CustomTestDataset(data)), ckpt_path=checkpoint_path)
		preds = [elem[0].argmax(axis=1).numpy() / 10 for elem in predictions]
		sample_name = ["TS" + str(i) for i in range(1, len(preds) + 1)]
		submission = pd.DataFrame({"sampleId": sample_name, "AAC": preds})
		submission["AAC"] = submission["AAC"].apply(lambda x: x[0])
		submission.to_csv(csv_logger.log_dir +'/submission.csv', index=False)
...
if __name__ == "__main__":
	...
	if args.use_best_hyperparams:
		if args.model in ['wpfs', 'fsnet', 'dietnetworks']:
			...
			if args.dataset=='your_custom_dataset':
				args.wpn_embedding_size = 128
				args.sparsity_gene_embedding_size = 128
